...
func GenerateNonNullTypeCaseStmts(callOfTypeFuncName string) []ast.Stmt {
	return []ast.Stmt{
		&ast.CaseClause{
			List: []ast.Expr{
				&ast.BasicLit{
			...
			Body: []ast.Stmt{
				&ast.AssignStmt{
					...
					Tok: token.ASSIGN,
					Rhs: []ast.Expr{
						ast.NewIdent("__TypeKind_NON_NULL"),
					},
				},
			},
		}, &ast.CaseClause{
...
func GenerateListTypeCaseStmts(callOfTypeFuncName string) []ast.Stmt {
	return []ast.Stmt{
		&ast.CaseClause{
			List: []ast.Expr{
				&ast.BasicLit{
